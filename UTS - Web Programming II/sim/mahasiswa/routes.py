...
from sim.mahasiswa.forms import login_M, mahasiswa_F, uts_M
...
@rmahasiswa.route("/uts_mhsw", methods=['GET','POST'])
def uts():
    form=uts_M()
    if form.validate_on_submit():
        flash(f'Anda Berhasil Kirim Komentar','success')
        return redirect(url_for('rmahasiswa.uts'))
    return render_template("uts.html", form=form)
